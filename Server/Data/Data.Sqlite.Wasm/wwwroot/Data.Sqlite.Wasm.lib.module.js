export function beforeStart(options, extensions) {
    window.dbSync = async function (cacheName) {
        const db = {
            init: false,
            cache: await caches.open(cacheName || 'embedded_db')
        };
        ...
        db.synchronizeDbWithCache = async function (filename) {
            ...
            console.log('Database downloading is available.');
            window.loadDbBackup = async () => {
                const cacheGetAttemptResponse = await db.cache.match(cachePath);
                const blob = await cacheGetAttemptResponse.blob();
